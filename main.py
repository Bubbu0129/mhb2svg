import argparse, json, pathlib, tempfile, urllib.parse, urllib.request, xml.etree.ElementTree, zipfile
...
def parse_args():
    ...
    group.add_argument('-l', '--link', type=str, help='MAXHUB shareable link')
    group.add_argument(      '--sid',  type=str, help='MAXHUB "s_id" (share ID)')
...
def parse_metadata(src_path):
    ...
    tree = xml.etree.ElementTree.parse(str(xml_path))
    root = tree.getroot()

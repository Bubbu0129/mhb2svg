...
def convert(input_path, output_path, isColor, padding, ratio):
    ...
    for ink in root.findall('.//Ink'):
        ...
        width *= ratio
        if isColor:
            color = ink.findtext('ForegroundColor')
        else:
            color = '#000000'
    ...
    with open(output_path, 'w') as f:
        ...
        if isColor: # Render fullscreen rectangle with background color
            f.write(
...
def process_slides(src_path, color, padding, ratio):
    ...
    for xml_path in xml_files:
        ...
        convert(str(xml_path), output_path, color, padding, ratio)
...
if __name__ == '__main__':
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        slides = process_slides(tmp_path, args.color, args.padding, args.ratio)
